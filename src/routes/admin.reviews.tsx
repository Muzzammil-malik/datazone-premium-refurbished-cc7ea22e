        ...
        {reviews.map((r) => (
          <div key={r.id} className="rounded-xl border bg-card p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {r.status !== "Approved" && <Button size="sm" variant="outline" onClick={() => { admin.saveReview({ ...r, status: "Approved" }); toast.success("Approved"); }}><Eye className="size-3.5" /> Approve</Button>}
              ...
              <Button size="sm" variant="outline" onClick={() => admin.saveReview({ ...r, featured: !r.featured })}><Sparkles className="size-3.5" /> {r.featured ? "Unfeature" : "Feature"}</Button>
              <Button size="sm" variant="ghost" className="text-destructive" onClick={() => { admin.deleteReview(r.id); toast.success("Deleted"); }}><Trash2 className="size-3.5" /></Button>
            </div>
          </div>
        ))}
      </div>
